    ...
    <Modal isOpen onClose={onClose} title="Add Payment Rule" size="md">
      <div className="space-y-4 p-1">
        <div>
          ...
          <input type="text" value={form.name} onChange={e => set('name', e.target.value)} className="input-field" placeholder="e.g. 30% deposit on booking" />
        </div>
        <div>
        ...
        {form.trigger === 'days_before_arrival' && (
          <div>
            ...
            <input type="number" min={1} value={form.days_before} onChange={e => set('days_before', Number(e.target.value))} className="input-field" />
          </div>
        )}
        <div className="grid grid-cols-2 gap-3">
          ...
          {(form.amount_type === 'percentage' || form.amount_type === 'fixed') && (
            <div>
              ...
              <input type="number" min={0} value={form.amount_value} onChange={e => set('amount_value', Number(e.target.value))} className="input-field" />
            </div>
          )}
        </div>
        <div className="grid grid-cols-2 gap-3">
