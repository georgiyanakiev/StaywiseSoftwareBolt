...
function StatusDot({ status }: { status: string }) {
  ...
  if (status === 'running') return <span className="flex h-2.5 w-2.5 rounded-full bg-blue-500 animate-pulse" />;
  return <span className="flex h-2.5 w-2.5 rounded-full bg-gray-300" />;
}

function SyncLogRow({ log }: { log: LodgifySyncLog }) {
  return (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {log.direction === 'inbound'
            ? <><ArrowDownToLine className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" /><span className="text-sm font-medium text-gray-800">Pulled reservations</span></>
            : <><ArrowUpFromLine className="h-3.5 w-3.5 text-sky-500 flex-shrink-0" /><span className="text-sm font-medium text-gray-800">Pushed availability</span></>
          }
...
export default function LodgifyPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [settingsRes, logsRes, reservationsRes] = await Promise.all([
        ...
        supabase.from('lodgify_sync_logs').select('*').eq('hotel_id', currentHotel.id).order('created_at', { ascending: false }).limit(30),
        supabase.from('reservations').select('id, status, created_at').eq('hotel_id', currentHotel.id).eq('booking_source', 'lodgify'),
      ]);

      setSettings(settingsRes.data as LodgifySettings | null);
      const logs = (logsRes.data || []) as LodgifySyncLog[];
      setSyncLogs(logs);
  ...
  const isConfigured = !!(settings?.api_key && (settings?.property_id || settings?.website_id));
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Lodgify Integration</h1>
            <p className="text-sm text-gray-500">Property management sync dashboard</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                To sync with Lodgify you need a Lodgify account with API access enabled.
                Once you have your API key and property ID, add them in <strong>Settings &gt; Lodgify</strong>.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <a
                  href="https://www.lodgify.com/api"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-800 underline hover:text-amber-900"
                >
                  Visit Lodgify API docs <ExternalLink className="h-3.5 w-3.5" />
                </a>
        ...
        <div className="rounded-xl bg-gray-50 border border-gray-200 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Enable in Settings &gt; Lodgify to start syncing</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/settings')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Lodgify Reservations', value: stats.totalSynced, icon: Calendar, color: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'This Week', value: stats.lastWeekReservations, icon: TrendingUp, color: 'text-sky-600', bg: 'bg-sky-50' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-4 rounded-lg border border-blue-100 bg-blue-50">
              <div className="w-9 h-9 rounded-lg bg-blue-100 flex items-center justify-center flex-shrink-0">
                <ArrowDownToLine className="h-5 w-5 text-blue-700" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-blue-900">Pull Reservations</p>
                <p className="text-xs text-blue-700 mt-0.5">Import new and updated bookings from Lodgify</p>
            ...
            <div className="flex items-start gap-4 p-4 rounded-lg border border-sky-100 bg-sky-50">
              <div className="w-9 h-9 rounded-lg bg-sky-100 flex items-center justify-center flex-shrink-0">
                <ArrowUpFromLine className="h-5 w-5 text-sky-700" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-sky-900">Push Availability & Rates</p>
                <p className="text-xs text-sky-700 mt-0.5">Update room availability and pricing on Lodgify</p>
              ...
              <button
                ...
                disabled={!!syncing || !isEnabled}
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-sky-600 text-white text-xs font-semibold hover:bg-sky-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
                {syncing === 'outbound'
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Website ID', value: settings?.website_id || '—' },
