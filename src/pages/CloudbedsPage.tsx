...
import type { CloudbedsSettings, CloudbedsSyncLog } from '../types';
...
function StatusDot({ status }: { status: string }) {
  ...
  if (status === 'running') return <span className="flex h-2.5 w-2.5 rounded-full bg-sky-500 animate-pulse" />;
  return <span className="flex h-2.5 w-2.5 rounded-full bg-gray-300" />;
}

function SyncLogRow({ log }: { log: CloudbedsSyncLog }) {
  return (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {log.direction === 'inbound'
            ? <><ArrowDownToLine className="h-3.5 w-3.5 text-sky-500 flex-shrink-0" /><span className="text-sm font-medium text-gray-800">Pulled reservations</span></>
          ...
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
            ...
            log.status === 'running' ? 'bg-sky-50 text-sky-700' :
...
export default function CloudbedsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [settingsRes, logsRes, reservationsRes] = await Promise.all([
        ...
        supabase.from('cloudbeds_sync_logs').select('*').eq('hotel_id', currentHotel.id).order('created_at', { ascending: false }).limit(30),
        supabase.from('reservations').select('id, status, created_at').eq('hotel_id', currentHotel.id).eq('booking_source', 'cloudbeds'),
      ]);

      setSettings(settingsRes.data as CloudbedsSettings | null);
      const logs = (logsRes.data || []) as CloudbedsSyncLog[];
      setSyncLogs(logs);
  ...
  const isConfigured = (settings?.property_id && settings?.client_id && settings?.client_secret) || !!settings?.api_key;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Cloudbeds Integration</h1>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                To sync with Cloudbeds you need a Cloudbeds account with API access.
                Once you have credentials, add them in <strong>Settings &gt; Cloudbeds</strong>.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <a
                  href="https://hotels.cloudbeds.com/api/docs"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-800 underline hover:text-amber-900"
                >
                  View Cloudbeds API documentation <ExternalLink className="h-3.5 w-3.5" />
                </a>
        ...
        <div className="rounded-xl bg-gray-50 border border-gray-200 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Enable in Settings &gt; Cloudbeds to start syncing</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/settings')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Cloudbeds Reservations', value: stats.totalSynced, icon: Calendar, color: 'text-sky-600', bg: 'bg-sky-50' },
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-4 rounded-lg border border-sky-100 bg-sky-50">
              <div className="w-9 h-9 rounded-lg bg-sky-100 flex items-center justify-center flex-shrink-0">
                <ArrowDownToLine className="h-5 w-5 text-sky-700" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-sky-900">Pull Reservations</p>
                <p className="text-xs text-sky-700 mt-0.5">Import new and updated bookings from Cloudbeds</p>
            ...
            <div className="flex items-start gap-4 p-4 rounded-lg border border-emerald-100 bg-emerald-50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-700 mt-0.5">Update room availability and pricing on Cloudbeds</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Auth Method', value: settings?.api_key ? 'API Key' : settings?.client_id ? 'OAuth2' : '—' },
