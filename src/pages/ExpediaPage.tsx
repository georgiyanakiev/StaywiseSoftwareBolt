...
import {
  RefreshCw, ArrowDownToLine, ArrowUpFromLine, CheckCircle2,
...
import type { ExpediaSettings, ExpediaSyncLog } from '../types';
...
function StatusDot({ status }: { status: string }) {
  ...
  if (status === 'running') return <span className="flex h-2.5 w-2.5 rounded-full bg-yellow-500 animate-pulse" />;
  return <span className="flex h-2.5 w-2.5 rounded-full bg-gray-300" />;
}

function SyncLogRow({ log }: { log: ExpediaSyncLog }) {
  return (
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          {log.direction === 'inbound'
            ? <><ArrowDownToLine className="h-3.5 w-3.5 text-yellow-600 flex-shrink-0" /><span className="text-sm font-medium text-gray-800">Pulled reservations</span></>
          ...
          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
            ...
            log.status === 'running' ? 'bg-yellow-50 text-yellow-700' :
...
export default function ExpediaPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [settingsRes, logsRes, reservationsRes] = await Promise.all([
        ...
        supabase.from('expedia_sync_logs').select('*').eq('hotel_id', currentHotel.id).order('created_at', { ascending: false }).limit(30),
        supabase.from('reservations').select('id, status, created_at').eq('hotel_id', currentHotel.id).eq('booking_source', 'expedia'),
      ]);

      setSettings(settingsRes.data as ExpediaSettings | null);
      const logs = (logsRes.data || []) as ExpediaSyncLog[];
      setSyncLogs(logs);
  ...
  const isConfigured = settings?.hotel_code && settings?.api_key && settings?.api_secret;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Expedia Integration</h1>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                To sync with Expedia you need an EQC (Expedia QuickConnect) account.
                Once approved, add your credentials in <strong>Settings &gt; Expedia</strong>.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <a
                  href="https://welcome.expediagroup.com/en/partners"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-800 underline hover:text-amber-900"
                >
                  Apply for Expedia Connectivity Partnership <ExternalLink className="h-3.5 w-3.5" />
                </a>
        ...
        <div className="rounded-xl bg-gray-50 border border-gray-200 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center">
              <span className="text-yellow-600 font-bold text-sm">E</span>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-700">Integration is disabled</p>
              <p className="text-sm text-gray-500">Enable in Settings &gt; Expedia to start syncing</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/settings')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Expedia Bookings', value: stats.totalSynced, icon: Calendar, color: 'text-yellow-600', bg: 'bg-yellow-50' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-4 rounded-lg border border-yellow-100 bg-yellow-50">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-yellow-900">Pull Reservations</p>
                <p className="text-xs text-yellow-700 mt-0.5">Import new and updated bookings from Expedia</p>
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-yellow-500 text-white text-xs font-semibold hover:bg-yellow-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              >
                {syncing === 'inbound'
            ...
            <div className="flex items-start gap-4 p-4 rounded-lg border border-emerald-100 bg-emerald-50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-700 mt-0.5">Update room availability and pricing on Expedia</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'POS ID', value: settings?.pos_id || '—' },
