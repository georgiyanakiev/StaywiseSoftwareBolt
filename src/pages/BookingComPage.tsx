...
import type { BookingComSettings, BookingComSyncLog } from '../types';
...
function SyncLogRow({ log }: { log: BookingComSyncLog }) {
  return (
...
export default function BookingComPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [settingsRes, logsRes, reservationsRes] = await Promise.all([
        ...
        supabase.from('booking_com_sync_logs').select('*').eq('hotel_id', currentHotel.id).order('created_at', { ascending: false }).limit(30),
        supabase.from('reservations').select('id, status, created_at').eq('hotel_id', currentHotel.id).eq('booking_source', 'booking.com'),
      ]);

      setSettings(settingsRes.data as BookingComSettings | null);
      const logs = (logsRes.data || []) as BookingComSyncLog[];
      setSyncLogs(logs);
  ...
  const isConfigured = settings?.property_id && settings?.client_id && settings?.client_secret;
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Booking.com Integration</h1>
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-amber-700 mt-1">
                To sync with Booking.com you need a Connectivity Partner account and API credentials.
                Once approved, add your credentials in <strong>Settings &gt; Booking.com</strong>.
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <a
                  href="https://join.booking.com/connectivity-partner"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-sm font-medium text-amber-800 underline hover:text-amber-900"
                >
                  Apply for Booking.com Connectivity Partnership <ExternalLink className="h-3.5 w-3.5" />
                </a>
        ...
        <div className="rounded-xl bg-gray-50 border border-gray-200 p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Enable in Settings &gt; Booking.com to start syncing</p>
            </div>
          </div>
          <button
            onClick={() => navigate('/settings')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total BDC Reservations', value: stats.totalSynced, icon: Calendar, color: 'text-blue-600', bg: 'bg-blue-50' },
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-4 p-4 rounded-lg border border-blue-100 bg-blue-50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-blue-700 mt-0.5">Import new and updated bookings from Booking.com</p>
            ...
            <div className="flex items-start gap-4 p-4 rounded-lg border border-emerald-100 bg-emerald-50">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-emerald-700 mt-0.5">Update room availability and pricing on Booking.com</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { label: 'Client ID', value: settings?.client_id ? `${settings.client_id.substring(0, 8)}...` : '—' },
