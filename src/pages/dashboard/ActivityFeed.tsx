...
const TYPE_CONFIG = {
  checkin: { icon: LogIn, color: 'text-emerald-600', bg: 'bg-emerald-50', label: 'Checked in' },
  checkout: { icon: LogOut, color: 'text-blue-600', bg: 'bg-blue-50', label: 'Checked out' },
  booking: { icon: CalendarPlus, color: 'text-amber-600', bg: 'bg-amber-50', label: 'New booking' },
          ...
          <div key={item.id} className="flex items-start gap-3 py-3 px-1 hover:bg-gray-50/60 rounded-lg transition-colors">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-500 mt-0.5">
                ...
                {item.roomNumber !== '—' && <span className="text-gray-400"> &middot; Room {item.roomNumber}</span>}
              </p>
            </div>
